...
```
ikanam_news/
├── assets/                       # Статические файлы (например, для SHAP-графиков)
│   └── shap_plots/
├── data/                         # Данные проекта
│   ├── external/                 # Внешние данные (например, исходный CSV с новостями)
│   │   └── data_for_llm/
│   ├── raw/                      # Данные после начальной обработки
│   └── user_data/                # Пользовательские данные
├── notebooks/                    # Jupyter ноутбуки для экспериментов, анализа и обучения
│   ├── train_dataset_getting.ipynb # Ноутбук для сбора, разметки и предобработки данных
│   └── train_model_process.ipynb   # Ноутбук для обучения моделей классификации
├── saved_models/                 # Сохраненные обученные модели и артефакты (токенизаторы, маппинги)
├── scripts/                      # Вспомогательные скрипты (например, для авторизации Telethon)
├── sessions/                     # Файлы сессий (например, для Telethon)
├── src/                          # Исходный код проекта
│   ├── bot/                      # Код Telegram-бота
│   ├── core/                     # Основная логика проекта
│   └── workers/                  # Код для воркеров или фоновых задач (например, gemini_workers.py)
